...
# ----------------------------------------------
# Wrapper for the Fortran subroutine GENERATECOMPLETEBINARYTREENEW

def generatecompletebinarytreenew(height, treeresult=None):
...
!  (which are words composed of the letters {0, 1}). The height of the tree must be at least 1.'''
    ...
    # Setting up "height"
    if (type(height) is not ctypes.c_int): height = ctypes.c_int(height)
    
    # Setting up "treeresult"
    if (treeresult is None):
        treeresult = numpy.zeros(shape=(20, 2**height), dtype=ctypes.c_char, order='F')
    elif ((not issubclass(type(treeresult), numpy.ndarray)) or
          (not numpy.asarray(treeresult).flags.f_contiguous) or
          (not (treeresult.dtype == numpy.dtype(ctypes.c_char)))):
        treeresult = numpy.asarray(treeresult, dtype=ctypes.c_char, order='F').reshape(-1)
    treeresult_dim_1 = ctypes.c_long(treeresult.shape[0])
    treeresult_dim_2 = ctypes.c_long(treeresult.shape[1])

    # Call C-accessible Fortran wrapper.
    clib.c_generatecompletebinarytreenew(ctypes.byref(height), ctypes.byref(treeresult_dim_1), ctypes.byref(treeresult_dim_2), ctypes.c_void_p(treeresult.ctypes.data))

    # Post-processing "treeresult"
    treeresult = ''.join(treeresult.astype(str))
